...
- **Add your own phases.** Each stage on the phase rail ends with a **＋** button (one for Analysis, one for Design). Click it, describe the modelling artifact you want — *CRC cards*, a *Volere requirements shell*, a *data dictionary*, a *package diagram* — and the studio authors a new phase for it: a board title, student hint chips, and Socratic instructions (it still elicits the artifact from the student, never generates it). The board draws the artifact one of two ways, chosen automatically to fit: **diagrams** render with Mermaid (a class diagram, sequence diagram, ER diagram, requirement diagram, and so on), while **inherently tabular artifacts** — CRC cards, Volere shells, data dictionaries, use-case textual specs — render as real **Markdown tables** rather than being forced into a diagram. (No third-party graphing library is loaded; fuller UML rendering via a server-side PlantUML engine is the planned Stage-C work — see `CLAUDE.md`.) Authoring uses your API key, so add one in Settings first.
